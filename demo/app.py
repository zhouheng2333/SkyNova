...
def generate_response(messages):
    ...
    #print(worker_addr)
    headers = {"User-Agent": "SkyNova-Chat Client"}
...
# App title
st.set_page_config(page_title="SkyNova")
...
<div class="gradient-text">SkyNova</div>
"""
if lan == "English":
    st.markdown(gradient_text_html, unsafe_allow_html=True)
    st.caption(
        '<p style="font-size:18px;">Convert multi-sensory RS data into interactive dialogue.</p>',
        unsafe_allow_html=True,
    )
else:
    st.markdown(
        gradient_text_html.replace("SkyNova", "书生·万象"), unsafe_allow_html=True
    )
...
with gallery_placeholder.container():
    ...
    if lan == "English":
        st.caption("Note: AI responses may contain errors. ")
    else:
        st.caption(
...
if lan == "English":
    ...
    if input_disable_flag:
        ...
        prompt = st.chat_input(
            "Send messages to SkyNova", disabled=input_disable_flag
        )
else:
    st.sidebar.button(
        "清空聊天记录",
    ...
    if input_disable_flag:
        ...
        prompt = st.chat_input("给 SkyNova 发送消息", disabled=input_disable_flag)
